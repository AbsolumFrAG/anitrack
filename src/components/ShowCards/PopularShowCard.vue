  ...
  <div v-if="data" class="popular-show-card-container">
    ...
    <div class="popular-show-card-data">
      ...
      >
        {{
          data.nextAiringEpisode
            ? `Épisode ${data.nextAiringEpisode.episode} de ${
                data.episodes || "TBD"
              }`
            : "Terminé le :"
        }}
      </div>
      <div v-else class="card-data-top">Commence dans</div>
      <div
        v-if="data.status !== 'NOT_YET_RELEASED'"
      ...
      >
        {{
          next_airing
            ? next_airing.day + " Jour, " + next_airing.hour + " heures"
            : end_date
        }}
      </div>
    ...
    setup(props) {
        ...
        onMounted(async () => {
            if (!props.data.nextAiringEpisode) {
                ...
                next_airing.value = await convert_next_airing(
